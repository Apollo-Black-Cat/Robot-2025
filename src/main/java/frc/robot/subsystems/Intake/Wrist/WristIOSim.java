...
public class WristIOSim implements WristIO {
  ...
  private final PIDController elevatorPID =
      new PIDController(
          IntakeConstants.WristConstants.simKp, 0.0, IntakeConstants.WristConstants.simKd);
  private final Encoder encoder = new Encoder(0, 1);
  private final SparkMax wristMotor =
      new SparkMax(IntakeConstants.WristConstants.wristMotorId, SparkMax.MotorType.kBrushless);
  ...
  public void updateInputs(WristIOInputs inputs) {
    if (closedLoop) {
      wristMotor.set(leftAppliedVolts);
    }
    ...
    inputs.rightCurrentAmps = new double[] {wristMotor.getOutputCurrent()};
